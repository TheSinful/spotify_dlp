...
Youtube::Youtube(std::string yt_api_key, DownloadConfig config)
...
std::string Youtube::make_search_request(Query query)
{
    ...
    if (res != CURLE_OK)
    {
        ...
        THROW_AND_LOG(CurlException,
                      ...
                      "YouTube API request failed with code: " + std::to_string(res));
    }
...
double Youtube::calculate_match_score(const SearchResult &result)
{
    if (this->is_track())
    {
        return calculate_track_score(result, std::get<TrackMetadata>(metadata));
    }
    else if (this->is_album())
    {
        return calculate_album_score(result, std::get<AlbumMetadata>(metadata));
    }
...
double Youtube::calculate_track_score(const SearchResult &result, const TrackMetadata &track)
{
    ...
    // Artist match (45%)
    std::string track_artists = normalize_string(query_builder.join_artists(track.artists));
...
double Youtube::calculate_album_score(const SearchResult &result, const AlbumMetadata &album)
{
    ...
    // Artist match (40%)
    std::string album_artists = normalize_string(query_builder.join_artists(album.artists));
    std::string result_artists = normalize_string(result.artist);
...
std::vector<URL> Youtube::search(AnyMetadata metadata)
{
    ...
    {
        PlaylistMetadata playlist = std::get<PlaylistMetadata>(metadata);
    ...
    {
        std::string response = this->make_search_request(query);
        std::vector<SearchResult> search_results = this->parse_response(response);
        ...
        {
            std::string base = "Skipping... Could not find a song that meets the minimum match score";
            LOG_INFO(base, base + "- Best match score too low (" + std::to_string(best_match.score) + ") for query: " + query);
        }
    }
